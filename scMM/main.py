...
parser.add_argument('--latent_dim', type=int, default=20, metavar='L',
...
# set up run path
runId = 'Dataset11_Human_Lymph_Node_A1'
experiment_dir = Path('../Data/' + runId + '/' + args.experiment)
experiment_dir.mkdir(parents=True, exist_ok=True)
runPath = experiment_dir
print(runPath)



#Data
dataset_path = "../Data/" + runId + "/"
...
if args.pre_trained:
    ...
    torch.save(t_id, str(runPath) + '/t_id.rar') #train data
    torch.save(s_id, str(runPath) + '/s_id.rar') #test data
...
if args.pre_trained:
    ...
    torch.save(args,str(runPath)+'/args.rar')
...
if __name__ == '__main__':
    ...
    with Timer('MM-VAE') as t:
        ...
        # for epoch in range(1, 9):
            ...
            if torch.isnan(torch.tensor([b_loss])):
                break          
            test(epoch, agg, W)
            save_vars(agg, str(runPath) + '/losses.rar')
    ...
    if args.analytics:
        ...
        # model.traverse(str(runPath), device)
