...
data_file = 'Data/Dataset10_Mouse_Brain_H3K27me3/'
...
for emb_name in emb_names:
    adata = mclust_R(adata, num_cluster=18, used_obsm=emb_name)
...
"""
plot_color = ['#F5BDC8', '#F1AEA1', '#FEE281', '#BACF90', '#D4EAE0', '#BAD4EA',
              '#DBC6E0', '#B99998', '#DCAF78', '#A7AF91', '#99A6B0', '#6B798D',
              '#EAD8C9', '#DD6E60', '#A57EB6', '#869F69', '#6C8CC0', '#A57EB6']
"""

plot_color = ["#458A74", "#F5A216", "#57AF37", "#41B9c1", "#008B8B",
              "#4E5689", "#6A8EC9", "#652884", "#8A7355", "#CC5B45",
              "#848484", "#EC3E31", "#B46DA9", "#F59B79", "#FBEA2E",
              "#A8D3A0", "#D65190", "#F898CB"]
...
adata.uns['RNA_clusters_colors'] = plot_color
adata.uns['ATAC_clusters_colors'] = plot_color


fig, axs = plt.subplots(1, 2, figsize=(2.8, 1.5), constrained_layout=True)
sc.pl.embedding(adata, basis='spatial', color=['RNA_clusters'], show=False, size=10, ax=axs[0],
              legend_fontsize=5)
# axs[0].invert_yaxis()
...
axs[0].set_title('RNA_clusters', fontsize=7)
plt.tight_layout(pad=0.2)

sc.pl.embedding(adata, basis='spatial', color=['ATAC_clusters'], show=False, size=10, ax=axs[1],
              legend_fontsize=5)
# axs[1].invert_yaxis()
...
axs[1].set_title('ATAC_clusters', fontsize=7)
plt.tight_layout(pad=0.2)
plt.savefig('Results/Mouse Brain/mouse_brain_ATAC_single_modality_embedding.pdf', dpi=300)




fig, axs = plt.subplots(1, 7, figsize=(8, 1.14),constrained_layout=True)
sc.pl.embedding(adata, basis='spatial', color=['MultiVI'], show=False, size=10, ax=axs[0],
...
sc.pl.embedding(adata, basis='spatial', color=['scMM'], show=False, size=10, ax=axs[2],
              legend_fontsize=0, legend_loc='on data')
# axs[2].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['GraphST'], show=False, size=10, ax=axs[3],
              legend_fontsize=0, legend_loc='on data')
# axs[3].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['Seurat'], show=False, size=10, ax=axs[4],
              legend_fontsize=0, legend_loc='on data')
# axs[4].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['SpatialGlue'], show=False, size=10, ax=axs[5],
              legend_fontsize=0, legend_loc='on data')
# axs[5].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['SpatialDDM'], show=False, size=10, ax=axs[6],
              legend_fontsize=0, legend_loc='on data')
# axs[5].invert_yaxis()
# axs[5].spines['right'].set_visible(False) # 去掉边框
# axs[5].spines['top'].set_visible(False)   # 去掉边框
...
plt.savefig('C:/Users/haiyu/Desktop/SpaDDM revised figures/mouse_brain_H3K27me3_embedding-8.pdf', dpi=300)
...
plt.show()
plt.savefig('Results/Mouse Brain/mouse_brain_ATAC_legend.pdf', dpi=300)
...
sc.pl.umap(Umap_MultiVI, color='MultiVI', ax=axs[0], show=False,
           size=8, legend_loc='on data', legend_fontsize=0)
# axs[0].spines['right'].set_visible(False) # 去掉边框
# axs[0].spines['top'].set_visible(False)   # 去掉边框
# axs[0].spines['left'].set_visible(False) # 去掉边框
# axs[0].spines['bottom'].set_visible(False)   # 去掉边框
...
sc.pl.umap(Umap_GraphST, color='GraphST', ax=axs[3], show=False,
           size=8, legend_loc='on data', legend_fontsize=0)
# axs[3].spines['right'].set_visible(False) # 去掉边框
# axs[3].spines['top'].set_visible(False)   # 去掉边框
# axs[3].spines['left'].set_visible(False) # 去掉边框
...
axs[6].get_yaxis().set_visible(False)
axs[6].get_xaxis().set_visible(False)
axs[6].set_title('', fontsize=13)

plt.savefig('C:/Users/haiyu/Desktop/SpaDDM revised figures/mouse_brain_H3K27me3_UMAP-8.pdf', dpi=300)