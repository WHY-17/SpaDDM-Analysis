...
data_file = 'Data/Dataset17_Simulation5/'
...
adata.obsm['Seurat'] = adata_seurat.obsm['X_wnn.umap']
...
adata.obs['Ground Truth'] = adata_ddm.obs['ground_truth'].astype('category')
adata.obs['GraphST'] = adata_graphst.obs['domain']


plot_color = ['#8582BD', '#4F99C9', '#A8D3A0', '#A6D0E6', '#F8B072']
...
adata.uns['Ground Truth_colors'] = plot_color
adata.uns['GraphST_colors'] = plot_color


fig, axs = plt.subplots(1, 8, figsize=(8, 1),constrained_layout=True)
sc.pl.embedding(adata, basis='spatial', color=['Ground Truth'], show=False, size=15, ax=axs[0],
...
sc.pl.embedding(adata, basis='spatial', color=['MultiVI'], show=False, size=15, ax=axs[1],
              legend_fontsize=0, legend_loc='on data')
# axs[1].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['totalVI'], show=False, size=15, ax=axs[2],
              legend_fontsize=0, legend_loc='on data')
# axs[2].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['scMM'], show=False, size=15, ax=axs[3],
              legend_fontsize=0, legend_loc='on data')
# axs[3].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['GraphST'], show=False, size=15, ax=axs[4],
              legend_fontsize=0, legend_loc='on data')
# axs[4].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['Seurat'], show=False, size=15, ax=axs[5],
              legend_fontsize=0, legend_loc='on data')
# axs[5].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['SpatialGlue'], show=False, size=15, ax=axs[6],
              legend_fontsize=0, legend_loc='on data')
# axs[5].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['SpatialDDM'], show=False, size=15, ax=axs[7],
              legend_fontsize=0, legend_loc='on data')
# axs[5].invert_yaxis()
# axs[6].spines['right'].set_visible(False) # 去掉边框
# axs[6].spines['top'].set_visible(False)   # 去掉边框
...
axs[7].set_title('', fontsize=15)

plt.savefig('C:/Users/haiyu/Desktop/SpaDDM revised figures/simulation5_embedding.pdf', dpi=300)
...
# Umap_SpatialDDM.obs['Ground Truth'] = adata.obs['Ground Truth']
...
Umap_SpatialDDM.uns['Ground Truth_colors'] = plot_color
...
fig, axs = plt.subplots(1, 8, figsize=(8,1),constrained_layout=True)
sc.pl.umap(adata, color='Ground Truth', ax=axs[0], show=False,
...
sc.pl.umap(Umap_GraphST, color='GraphST', ax=axs[4], show=False,
           size=10, legend_loc='on data', legend_fontsize=0)
# axs[4].spines['right'].set_visible(False) # 去掉边框
# axs[4].spines['top'].set_visible(False)   # 去掉边框
# axs[4].spines['left'].set_visible(False) # 去掉边框
...
plt.savefig('C:/Users/haiyu/Desktop/SpaDDM revised figures/simulation5_UMAP.pdf', dpi=300)
...
if isinstance(ax, list):
    ax = ax[0]
# 3. 创建一个新的图例画布
fig_legend = plt.figure(figsize=(6, 1.2))  # 根据图例数量自定义尺寸
...
plt.show()
plt.savefig('C:/Users/haiyu/Desktop/SpaDDM revised figures/plot_legend.pdf', dpi=300)