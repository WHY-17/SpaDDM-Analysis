...
data_file = 'Data/Dataset6_Mouse_Thymus4/'
...
# plot_color = ['#99A6B0', '#6B798D','#EAD8C9', '#DD6E60', '#A57EB6', '#869F69', '#6C8CC0', '#A57EB6']
plot_color = ['#F59B7B', '#ED8828','#A4DDD3', '#81B21F', '#8D73BA', '#ABD7EC', '#33ABC1', '#FCC41E']
# plot_color = ["#458A74", "#F5A216", "#57AF37", "#41B9c1", "#008B8B",
#               "#6A8EC9", "#B46DA9", "#CC5B45"]
...
sc.pl.embedding(adata, basis='spatial', color=['StabMap'], show=False, size=10, ax=axs[3],
              legend_fontsize=0, legend_loc='on data')
axs[3].invert_yaxis()
...
sc.pl.embedding(adata, basis='spatial', color=['GraphST'], show=False, size=10, ax=axs[4],
              legend_fontsize=0, legend_loc='on data')
axs[4].invert_yaxis()
...
# plt.savefig('C:/Users/haiyu/Desktop/SpaDDM revised figures/mouse_thymus4_embedding-8.pdf', dpi=300)
...
legend_ax.legend(
    ...
    frameon=False,
    fontsize=8,
    ncol=8,           # 图例横向排列列数（根据需要调整）
    handletextpad=0.8,
    columnspacing=1.5
)
plt.show()
plt.savefig('Results/Mouse Thymus/mouse_thymus_legend.pdf', dpi=300)
"""
...
Umap_StabMap.uns['StabMap_colors'] = plot_color
...
sc.pl.umap(Umap_MultiVI, color='MultiVI', ax=axs[0], show=False,
           size=5, legend_loc='on data', legend_fontsize=0)
# axs[0].spines['right'].set_visible(False) # 去掉边框
# axs[0].spines['top'].set_visible(False)   # 去掉边框
# axs[0].spines['left'].set_visible(False) # 去掉边框
# axs[0].spines['bottom'].set_visible(False)   # 去掉边框
...
plt.savefig('C:/Users/haiyu/Desktop/SpaDDM revised figures/mouse_thymus4_UMAP-8.pdf', dpi=300)